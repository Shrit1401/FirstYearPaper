...
export default function RepeatV2Client({ initialPaperId = "", initialSource = "" }: { initialPaperId?: string; initialSource?: string }) {
  ...
  return <main className="repeat-app dark">
    ...
    <aside ref={sidebarRef} id="repeat-subjects" className="repeat-sidebar" data-open={sidebarOpen} aria-label="Study subjects" role={modal === "sidebar" ? "dialog" : undefined} aria-modal={modal === "sidebar" ? true : undefined} tabIndex={-1}>
      <Link prefetch={false} href="/repeat" className="repeat-brand" aria-label="Repeat 2.0 home"><span className="repeat-brand-symbol"><RotateCcw size={19} strokeWidth={2.2} /></span><span className="repeat-brand-name">repeat<span className="repeat-version ml-2">2.0</span></span></Link>
      <p className="repeat-sidebar-caption">Past papers · AI-transcribed questions</p><div className="px-2 mb-3"><label className="repeat-label" htmlFor="exam-filter">Exam</label><select id="exam-filter" className="w-full rounded-lg border bg-black p-2" value={exam} onChange={e=>{setExam(e.target.value);resetSelection();}}><option value="MIDSEM">Midsem</option>{year === 1 && <><option value="all">All exams</option><option value="ENDSEM">Endsem</option><option value="REGULAR">Regular</option><option value="MAKEUP">Makeup</option></>}</select><Link prefetch={false} href="/midsem" className="mt-3 block text-xs underline">Free papers + worked solutions</Link></div>
      ...
      <div className="repeat-sidebar-bottom"><Link prefetch={false} href="/" className="inline-flex items-center gap-2"><ArrowLeft size={12} /> Back to Papers</Link></div>
    ...
    <div ref={workspaceRef} className="repeat-workspace">
      <header ref={topbarRef} className="repeat-topbar"><div className="repeat-topbar-context">
        <Link prefetch={false} href="/repeat" className="repeat-brand repeat-topbar-brand" aria-label="Repeat 2.0 home"><span className="repeat-brand-symbol"><RotateCcw size={18} strokeWidth={2.2} /></span><span className="repeat-brand-name">repeat<span className="repeat-version">2.0</span></span></Link>
        ...
        <div className="flex items-center gap-2">
          <Link prefetch={false} href="/profile" className="repeat-account-chip" title={profile?.email ?? "Profile"}><UserRound size={13} /><span>{profile?.name?.split(" ")[0] || "Account"}</span></Link>
      ...
      <div className="repeat-columns">
        ...
        <aside ref={tutorRef} id="repeat-study-tutor" className="repeat-tutor" data-open={tutorOpen} aria-label="Study tutor" role={modal === "tutor" ? "dialog" : undefined} aria-modal={modal === "tutor" ? true : undefined} tabIndex={-1}>
          ...
          <div className="repeat-tutor-scroll" ref={tutorScroll} onScroll={() => { const element = tutorScroll.current; if (element) shouldScroll.current = element.scrollHeight - element.scrollTop - element.clientHeight < 100; }}>
            {currentChat?.locked ? <div className="repeat-tutor-locked"><p className="repeat-tutor-locked-lead">Your session ended. <Link prefetch={false} href="/auth?next=/repeat" className="underline underline-offset-4">Sign in again</Link> to keep going.</p></div>
